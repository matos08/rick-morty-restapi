...
class CreateCharacterUseCase:
    ...
    async def validate(self):
        character = await self._repository.get_or_none(name=self._payload.name, gender=self._payload.gender)
        if character:
            raise HTTPException(detail="Character already exist", status_code=status.HTTP_400_BAD_REQUEST)
